#import "theme.typ": *
#show: slides.with(
  title: [Множества],
  subtitle: "Дискретная математика",
  date: "Осень 2026",
  authors: "Константин Чухарев",
)

#import "diagrams/m04.typ": (
  points-grid, venn-difference, venn-intersection, venn-subset, venn-union,
)

= Наивная теория множеств
#focus-slide(
  epigraph: [Множество --- это Многое, мыслимое как Одно.],
  epigraph-author: [Георг Кантор],
)

== Множество

Группировать объекты --- базовое действие математики: буквы алфавита, вершины графа, простые числа.

#definition[
  Множество --- неупорядоченная коллекция различных объектов.
  Объекты, входящие в множество, называются его _элементами_.
]

#example[
  $A = {2, 3, 5, 7}$ --- первые четыре простых числа.
  ${2, 3} = {3, 2}$ и ${2, 2, 3} = {2, 3}$: порядок и повторы не влияют.
]

== Множество как коробка

#important[
  Множество --- "коробка".

  Главное, что можно сделать с элементом, --- проверить, лежит ли он внутри.
]

== Принадлежность

#definition[
  - Запись $a in A$ означает "a принадлежит множеству A".
  - Запись $a in.not A$ означает "a не принадлежит A".
]

#example[Вложенные множества][
  $B = {a, {b}}$: $a in B$ и ${b} in B$, но $b in.not B$ --- $b$ лежит внутри вложенного множества.
]

#important[
  Принадлежность проверяет только _прямых_ элементов.

  Внутрь вложенных множеств она не заглядывает.
]

== Способы задания

Множество можно задать перечислением или свойством.

#example[Перечисление][
  - ${1, 2, 3}$.
  - ${alpha, beta, gamma}$.
  - $emptyset = {}$ --- пустое множество.
]

#example[Свойством][
  - ${x in NN mid(|) x "чётно"}$ --- множество чётных натуральных чисел.
  - ${x in RR mid(|) x^2 = 2}$ --- множество корней уравнения $x^2 = 2$.
]

#important[
  Свойство задаёт множество --- это мост от логики.

  Предикат $P(x)$ из прошлых лекций превращается в множество ${x mid(|) P(x)}$.
]

== Важные множества

- $NN = {0, 1, 2, ...}$ --- натуральные числа.
- $ZZ = {..., -2, -1, 0, 1, 2, ...}$ --- целые.
- $QQ = {a\/b mid(|) a in ZZ, b in NN, b != 0}$ --- рациональные.
- $RR$ --- действительные: вся координатная прямая.
- $BB = {#False, #True}$ --- булевы значения.

== Равенство множеств

#definition[
  Множества $A$ и $B$ равны, если у них одни и те же элементы.
  Формально: $A = B$ iff $forall x. thin (x in A) iff (x in B)$.
]

Равенство можно проверить двойным включением: $A = B$ iff $A subset.eq B$ and $B subset.eq A$.

Знак $subset.eq$ разберём на следующем слайде.

#example[
  ${1, 2} = {2, 1} = {1, 2, 2}$ --- порядок и повторы не влияют.
]

#note[
  Свойство "равны, если одинаковы элементы" называется _экстенсиональностью_.

  В аксиоматической теории множеств оно постулируется аксиомой.
]

== Подмножество

#definition[
  Множество $A$ --- _подмножество_ $B$, если каждый элемент $A$ лежит в $B$.
  Пишут $A subset.eq B$.
  Формально: $A subset.eq B$ iff $forall x. thin (x in A) -> (x in B)$.
]

Если $A subset.eq B$ и $A != B$, то $A$ --- _собственное_ подмножество: $A subset B$.

#example[
  - $emptyset subset.eq A$ для любого $A$: у пустого множества нет элементов, поэтому условие "каждый его элемент лежит в $A$" выполнено пустым образом.
  - Множество чётных чисел --- собственное подмножество $ZZ$: $ZZ_"even" subset ZZ$.
]

#important[
  В определении подмножества работает импликация из логики.

  $x in A -> x in B$ --- ровно то "если ... то", которое мы изучили.
]

== Операции над множествами

Зафиксируем _универсум_ $U$ --- множество всех объектов текущей задачи.

Дополнение считается относительно него.

#table(
  columns: 2,
  align: (left, left),
  stroke: (x, y) => if y == 0 { (bottom: 0.8pt) },
  table.header([*Операция*], [*Определение*]),
  [Объединение], [$A union B = {x mid(|) x in A or x in B}$],
  [Пересечение], [$A inter B = {x mid(|) x in A and x in B}$],
  [Разность], [$A setminus B = {x mid(|) x in A and x in.not B}$],
  [Симметрическая разность],
  [$A symdiff B = (A setminus B) union (B setminus A)$],

  [Дополнение], [$overline(A) = {x mid(|) x in.not A}$ относительно $U$],
)

#example[
  $A = {1, 2, 3}$, $B = {2, 3, 4}$:
  $A setminus B = {1}$, $A symdiff B = {1, 4}$.
]

== Диаграммы Венна

Диаграммы Венна --- наглядное представление операций над множествами.

#grid(
  columns: 2,
  column-gutter: 1em,
  row-gutter: 1em,
  align(center, venn-union), align(center, venn-intersection),

  align(center, venn-difference), align(center, venn-subset),
)

== Пределы диаграмм

#note[
  Диаграммы Венна --- интуиция, а не доказательство.

  Для доказательства нужны определения и логика.
]

== Соответствие со связками

Операции над множествами --- логические связки в другой записи.

#table(
  columns: 2,
  align: (center, center),
  stroke: (x, y) => if y == 0 { (bottom: 0.8pt) },
  table.header([*Множества*], [*Логика*]),
  [$union$], [$or$],
  [$inter$], [$and$],
  [$overline(A)$], [$not$],
  [$A subset.eq B$], [$x in A -> x in B$],
)

#important[
  Каждому закону логики соответствует тождество множеств.

  Формально: замена $and$ на $inter$, $or$ на $union$, $not$ на $overline(A)$ переносит закон без изменений.
]

== Законы алгебры множеств

Каждая строка --- логический закон в записи множеств.

#table(
  columns: 2,
  align: (left, left),
  inset: 4pt,
  stroke: (x, y) => if y == 0 { (bottom: 0.8pt) },
  table.header([*Закон*], [*Формулы*]),
  [Идемпотентность], [$A union A = A$, $A inter A = A$],
  [Коммутативность], [$A union B = B union A$, $A inter B = B inter A$],
  [Ассоциативность],
  [$(A union B) union C = A union (B union C)$, $(A inter B) inter C = A inter (B inter C)$],

  [Дистрибутивность],
  [$A union (B inter C) = (A union B) inter (A union C)$, $A inter (B union C) = (A inter B) union (A inter C)$],

  [Поглощение], [$A union (A inter B) = A$, $A inter (A union B) = A$],
  [Тождество], [$A union emptyset = A$, $A inter U = A$],
  [Дополнение], [$A union overline(A) = U$, $A inter overline(A) = emptyset$],
)

#important[
  Тождества отличаются от логических законов только именами операций.

  Доказательства не нужны: они уже доказаны для логики.
]

== Де Морган для множеств

#theorem[Законы де Моргана для множеств][
  $
    overline(A inter B) = overline(A) union overline(B) \
    overline(A union B) = overline(A) inter overline(B)
  $
]

== Доказательство де Моргана

#proof[
  Докажем первый закон цепочкой эквивалентностей.
  Третий переход --- закон де Моргана из логики:
  $
    x in overline(A inter B)
    iff x in.not (A inter B)
    iff not (x in A and x in B)
    iff (x in.not A) or (x in.not B)
    iff x in overline(A) union overline(B)
  $
  Значит, множества $overline(A inter B)$ и $overline(A) union overline(B)$ совпадают.

  Второй закон доказывается аналогично.
]

#important[
  Закон де Моргана из логики переносится на множества заменой связок:

  $overline(A inter B) = overline(A) union overline(B)$.
]

== Тождества множеств

Законы алгебры множеств позволяют доказывать тождества.

Докажем, что $A setminus (B union C) = (A setminus B) inter (A setminus C)$.

#proof[
  Цепочка эквивалентностей для произвольного элемента $x$:
  $
    x in A setminus (B union C)
    &iff x in A and x in.not (B union C) \
    &iff x in A and (x in.not B and x in.not C) \
    &iff (x in A and x in.not B) and (x in A and x in.not C) \
    &iff x in (A setminus B) inter (A setminus C)
  $
  Множества совпадают, потому что совпадают их элементы.
]

#important[
  Второй переход --- закон де Моргана из логики в действии.
]

== Циклическое доказательство

#theorem[
  Для любых множеств $A$, $B$, $C$:
  $
    A subset.eq B union C quad iff quad A setminus C subset.eq B quad iff quad A inter overline(B) subset.eq C
  $
]

#proof[
  Докажем по кругу: $1 imply 2 imply 3 imply 1$.

  *$1 imply 2$.* Если $x in A setminus C$, то $x in A subset.eq B union C$ и $x in.not C$, значит, $x in B$.

  *$2 imply 3$.* Если $x in A inter overline(B)$, то $x in A$ и $x in.not B$. При $x in.not C$ вышло бы $x in A setminus C subset.eq B$.

  *$3 imply 1$.* Если $x in A$, то либо $x in B$, либо $x in A inter overline(B) subset.eq C$. В обоих случаях $x in B union C$.
]

#important[
  Порядок обхода круга произволен: годится любая перестановка.
]

== Ложная аналогия

#note[
  Ложная аналогия с арифметикой: $A setminus (B union C) != (A setminus B) union (A setminus C)$.

  Проверьте на $A = {1, 2, 3}$, $B = {2}$, $C = {3}$: слева ${1}$, справа ${1, 2, 3}$.
  Верный закон --- с пересечением: $A setminus (B union C) = (A setminus B) inter (A setminus C)$.
]

== Парадокс Рассела

Назовём множество _нормальным_, если оно не содержит себя: $A in.not A$.

#important[
  Множество всех нормальных множеств $R = {A mid(|) A in.not A}$ даёт противоречие в обе стороны.
]

#proof[
  Если $R in R$, то по определению оно нормально, то есть $R in.not R$.

  Если $R in.not R$, то свойство нормальности выполнено, значит, $R in R$.

  Обе ветви противоречивы: множества всех нормальных множеств не существует.
]

#note[
  Рассел сообщил о противоречии Фреге, и тот во втором томе признал, что основания его арифметики пошатнулись.
]

== Аксиома выделения

#definition[
  *Аксиома выделения* (Цермело, 1908): для множества $A$ и свойства $P$ существует множество ${x in A mid(|) P(x)}$.
]

#important[
  Множество собирается только внутри готового $A$.

  Свойство без универсума --- вроде $A in.not A$ --- множества не задаёт.
]

#example[
  - ${x in ZZ mid(|) x > 0}$ --- положительные целые.
  - ${x in NN mid(|) x mod 2 = 0}$ --- чётные натуральные.
]

== Аксиомы ZFC

Система Цермело--Френкеля с аксиомой выбора:

+ *Объёмность*: множества с одними и теми же элементами равны.
+ *Пустое множество*: $emptyset$ существует.
+ *Пары*: для любых $a$ и $b$ существует ${a, b}$.
+ *Объединение*: у любого семейства множеств есть объединение.
+ *Булеан*: для $A$ существует $power(A)$.
+ *Бесконечность*: бесконечное множество существует.
+ *Выделение*: из $A$ и свойства $P$ существует ${x in A mid(|) P(x)}$.
+ *Замена*: образ множества под функциональным отношением --- множество.
+ *Основание*: в любом непустом множестве есть минимальный элемент.
+ *Выбор*: у семейства непустых множеств есть функция выбора.

#note[
  Выделение закрывает парадокс Рассела, основание запрещает самопринадлежность и цепочки $... in x_2 in x_1 in x_0$.
]

== Булеан

#definition[
  Булеан (_power set_) множества $A$ --- множество всех его подмножеств:
  $power(A) = {S mid(|) S subset.eq A}$.
]

#example[
  $power({a, b}) = {emptyset, {a}, {b}, {a, b}}$.
]

#theorem[
  Для конечного множества из $n$ элементов $abs(power(A)) = 2^n$.
]

== Мощность булеана

#proof[
  Для каждого из $n$ элементов есть два выбора: включить в подмножество или нет.
  Независимые выборы дают $2^n$ подмножеств.
]

#important[
  Булеан растёт экспоненциально.

  Уже при $n = 20$ подмножеств больше миллиона.
]

== Индукция по размеру

#proof[
  Докажем $abs(power(A)) = 2^n$ индукцией по $n = abs(A)$.

  *База.* При $n = 0$ имеем $A = emptyset$ и $power(A) = {emptyset}$, значит, $abs(power(A)) = 1 = 2^0$.

  *Шаг.* Пусть формула верна для множеств размера $k$, и $abs(A) = k + 1$.
  Выделим $a in A$ и положим $A' = A setminus {a}$.

  Подмножества $A$ делятся на две части.
  Без $a$ --- это $power(A')$, их $2^k$.
  С $a$ --- вида $S union {a}$ для $S subset.eq A'$, их тоже $2^k$.

  Всего $2^k + 2^k = 2^(k+1)$.
]

== Декартово произведение

#definition[
  Декартово произведение множеств $A$ и $B$:
  $A times B = { (a, b) mid(|) a in A and b in B }$.
]

#example[
  $A = {1, 2}$, $B = {x, y}$:
  $A times B = { (1, x), (1, y), (2, x), (2, y) }$.
]

#theorem[
  Для конечных $A$ и $B$ выполнено $abs(A times B) = abs(A) dot abs(B)$.
]

#proof[
  Для первой компоненты пары есть $abs(A)$ вариантов, для второй --- $abs(B)$.
  Независимые выборы перемножаются: $abs(A times B) = abs(A) dot abs(B)$.
]

== Декартово произведение в приложениях

#grid(
  columns: (1fr, auto),
  column-gutter: 1em,
  align: (top + left, top + center),
  [
    Точка плоскости --- пара $(a, b)$.
    Произведение --- все точки такой сетки.

    #important[
      Декартово произведение --- основа координатной плоскости и кортежей в базах данных.
    ]
  ],
  points-grid,
)

== Разбиения

#definition[
  Разбиение (_partition_) множества $X$ --- набор непустых попарно непересекающихся (_disjoint_) подмножеств, покрывающих $X$.
]

#example[
  ${{0, 2, 4, ...}, {1, 3, 5, ...}}$ --- разбиение $NN$ на чётные и нечётные числа.
]

#important[
  Разбиения тесно связаны с отношениями эквивалентности --- следующая тема.
]

== Что не разбиение

#example[
  Три условия определения --- три способа нарушить их для $X = {1, 2, 3, 4}$:
  - ${{1, 2}, {2, 3}, {4}}$ --- блоки пересекаются по элементу $2$.
  - ${{1, 2}, {3}}$ --- элемент $4$ не покрыт.
  - ${{1, 2}, {3, 4}, emptyset}$ --- пустой блок.
]

== Множества и SQL

Таблица в базе данных --- подмножество декартова произведения.
- Строка --- кортеж.
- Колонка --- компонент кортежа.

#note[
  - SELECT --- проекция.
  - WHERE --- отбор по предикату.
  - JOIN --- декартово произведение с условием.
  - UNION, INTERSECT, EXCEPT --- объединение, пересечение, разность.
  SQL --- теория множеств в продакшене.
]

== Типы как множества

Тип --- множество допустимых значений.

#note[
  - $"bool" = {#False, #True}$.
  - $"uint8" = {0, 1, ..., 255}$.
  - Тип-произведение (struct) --- декартово произведение.
  - Тип-сумма --- дизъюнктное объединение.
  - $"Option" la T ra$ --- тип-сумма: значение "None" или "Some"(T).
]

== Битовые маски

Множество $S subset.eq {1, ..., n}$ можно закодировать битовой строкой длины $n$.

#note[
  - $i$-й бит равен 1, если $i in S$.
  - Объединение --- побитовое OR.
  - Пересечение --- побитовое AND.
  - Дополнение --- побитовое NOT.
]

#example[
  Права доступа (rwx) --- три бита:
  - read.
  - write.
  - execute.

  Маска $110_2$ означает чтение и запись без исполнения.
]

== Хеширование

Хеш-функция отображает ключи в конечное множество корзин.

#note[
  Коллизия --- два ключа попали в одну корзину.

  Если ключей больше, чем корзин, коллизия неизбежна: это принцип Дирихле.
]

#important[
  Простейшая форма принципа Дирихле: если $n + 1$ кроликов в $n$ клетках, то есть клетка с двумя кроликами.
]

== Что мы поняли?

#important[
  - Множество --- "коробка" с единственным вопросом: лежит ли $x$ внутри.
  - Операции над множествами --- логические связки в другой записи. Законы переносятся без нового доказательства.
  - Булеан $power(A)$ растёт экспоненциально: $2^n$ подмножеств.
  - Дальше всё строится на множествах: отношения --- множества пар, функции --- их частный случай.
]

== Вопросы для самопроверки

- Почему ${2, 2, 3} = {2, 3}$?

- Верно ли, что $emptyset in emptyset$?
  А что насчёт $emptyset subset.eq emptyset$?

- Запишите множество чётных чисел через предикат.

- Чему равен булеан множества ${emptyset}$?

- Придумайте разбиение $ZZ$ на три класса.

- Докажите закон де Моргана $overline(A union B) = overline(A) inter overline(B)$.
