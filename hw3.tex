...
\begin{tasks}
    ...
    \item For each given function $f_i$ of 4 arguments, draw the Karnaugh map and use it to find BCF, minimal DNF, and minimal CNF. Additionally, construct ANF (Zhegalkin polynomial) using either the tabular (\enquote{triangle}) method or the Pascal method --- use each method at least once.
    ...
    \begin{subtasks}
        ...
        \item $\mathtt{majority}(X_1, X_2, X_3)$ \footnote{Majority function\Href{https://en.wikipedia.org/wiki/Majority_function} is a Boolean function that is 1 iff the majority (more than half) of the inputs are 1.}
    ...
    %% Task: Prove that Zhekalkin basis is complete.
    \item Show \--- without using Post's criterion \--- that the Zhegalkin basis $\Set{\xor, \land, 1}$ is functionally complete.
    %, \ie its clone\Href{https://en.wikipedia.org/wiki/Clone_(algebra)} is equal to the set $\mathbb{F}$ of all Boolean functions.
...
%     \end{defbox}
    ...
    %% Task: BDD.
    \item Binary Decision Diagram\Href{https://en.wikipedia.org/wiki/Binary_decision_diagram} is a representation of a Boolean function as a directed acyclic graph, which consists of \emph{decision} nodes and two \emph{terminal} nodes (0~and~1).
    ...
    A path from the root node to the 1-terminal (0-terminal) represents an assignment (a cube) for which the represented Boolean function is true (false, respectively).
    ...
    Determine whether the OBDD can be reduced by using a different variable order.
