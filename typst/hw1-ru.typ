...
- $Active = {"malware", "phishing", "ddos", "ransomware", "botnet"}$
  #h(1fr) (в настоящее время _активно_ обнаруженные угрозы)
...
+ Определите, является ли ${Critical, High, Medium}$ разбиением (разложением) множества $Active$.
...
Рассмотрите _предпочтения пользователей_ как множества жанров, которые им нравятся.
Например, если Анна любит запутанные сюжеты, её множество предпочтений: $A = {"sci-fi", "thriller"}$.
...
== Задача 4: Логика и тождества множеств
...
*Часть (c):*
Игрок начинает в позиции $P_0 = pair(2, 6)$ и движется согласно векторам $v_1 = pair(4, 0)$, #box($v_2 = pair(1, -2)$) и $v_3 = pair(-4, -3)$.
+ Вычислите позицию игрока после каждого хода: $P_i = P_(i-1) + v_i$.
...
Эти математические головоломки исследуют аналогичные самореферентные концепции, которые встречаются в программировании, логике и даже философии.
...
*Часть (b):*
Рассмотрите более сложную систему:
$
  A & = {1, card(B), card(C)} \
  B & = {2, card(A), card(C)} \
  C & = {1, 2, card(A), card(B)}
...
Является ли человек ростом 180 см высоким?
Является ли 10°C тёплой температурой?
...
Перечислите все элементы, запускающие предупреждение, с их степенями и кратко прокомментируйте компромисс между использованием чётких и нечётких порогов.
...
- Для доказательств сформулируйте, что вы доказываете, приведите ясные логические шаги и завершите символом QED или $square$.
