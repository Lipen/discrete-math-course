...
\begin{tasks}
    ...
    \begin{terms}
        ...
        \item $R^1 = R$, $R^{k+1} = R^k \circ R$ is a \textit{compositional power}\footnote{Note: this is \textit{not a Cartesian power}, despite of the same notation~$R^n$. Another possible notation for compositional power~is~$R^{\circ n}$, but it is too wild to use it here.},
        \item $R \circ S = \Set{\Pair{x,y} \given \exists z : (x \rel[R] z) \land (z \rel[S] y)}$ is a \textit{composition} (\textit{product}) of relations $R$ and $S$.
    \end{terms}
