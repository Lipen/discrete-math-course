...
\begin{tasks}
    ...
    \item For each given set $M_i$ and relation $R_i \subseteq M^2$, determine whether it is \textit{reflexive}, \textit{irreflexive}, \textit{coreflexive}, \textit{symmetric}, \textit{antisymmetric}, \textit{asymmetric}, \textit{transitive}, \textit{antitransitive}, \textit{connex}, \textit{right/left Euclidean}, \textit{dense}.
    ...
    %% Task: Proof some statements about relation properties.
    ...
    \item Let $R_{\theta}$ be a relation of $\theta$-similarity of finite sets defined as follows\footnotemark{}: $A \rel[R_{\theta}] B \iff \Jac(A,B) \geq \theta$, where $\Jac(A,B) = \frac{\card{A \intersection B}}{\card{A \union B}}$ is a Jaccard index.
    \footnotetext{A set $A$ is said to be $\theta$-similar to $B$ \textit{iff} the Jaccard index $\Jac(A,B)$ for these sets is at least $\theta$. Obviously, $\theta \in [0; 1] \subseteq \Real$.}
    ...
    \begin{subtasks}
        ...
        \item Find the quotient set of $\powerset{X}$ by $R$.
    \end{subtasks}
    ...
    % \item Show that the transitive closure of $R$ is in fact transitive.
