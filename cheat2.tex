...
\subsection{Некоторые свойства бинарных отношений}
...
\begin{tabular}{l @{\hspace{1em}} L @{\hspace{1em}} l}
    \toprule
    \thead{Свойство} & \thead{\text{Формальное определение}} & \thead{Отношение} \\
    \midrule
    Рефлексивность & \forall x \in M : x \rel x & \href{https://en.wikipedia.org/wiki/Reflexive_relation}{Reflexive} \\
    Иррефлексивность & \forall x \in M : \neg (x \rel x) & \href{https://en.wikipedia.org/wiki/Irreflexive_relation}{Irreflexive} \\
    Корефлексивность & \forall x,y \in M : (x \rel y) \implies (x = y) & \href{https://en.wikipedia.org/wiki/Coreflexive_relation}{Coreflexive} \\
    Симметричность & \forall x,y \in M : (x \rel y) \implies (y \rel x) & \href{https://en.wikipedia.org/wiki/Symmetric_relation}{Symmetric} \\
    Антисимметричность & \forall x,y \in M : (x \rel y) \land (y \rel x) \implies (x = y)) & \href{https://en.wikipedia.org/wiki/Antisymmetric_relation}{Antisymmetric} \\
    Асимметричность & \forall x,y \in M : (x \rel y) \implies \neg (y \rel x) & \href{https://en.wikipedia.org/wiki/Asymmetric_relation}{Asymmetric} \\
    Транзитивность & \forall x,y,z \in M : (x \rel y) \land (y \rel z) \implies (x \rel z) & \href{https://en.wikipedia.org/wiki/Transitive_relation}{Transitive} \\
    Антитранзитивность & \forall x,y,z \in M : (x \rel y) \land (y \rel z) \implies \neg (x \rel z)) & \href{https://en.wikipedia.org/wiki/Antitransitive}{Antitransitive} \\
    Евклидовость (правая) & \forall x,y,z \in M : (x \rel y) \land (x \rel z) \implies (y \rel z) & \href{https://en.wikipedia.org/wiki/Euclidean_relation}{Right Euclidean} \\
    Евклидовость (левая) & \forall x,y,z \in M : (y \rel x) \land (z \rel x) \implies (y \rel z) & \href{https://en.wikipedia.org/wiki/Euclidean_relation}{Left Euclidean} \\
    Связность & \forall x,y \in M : (x \neq y) \implies (x \rel y) \lor (y \rel x) & \href{https://en.wikipedia.org/wiki/Connected_relation}{Connex} \\
    Связность (сильная) & \forall x,y \in M : (x \rel y) \lor (y \rel x) & \href{https://en.wikipedia.org/wiki/Connected_relation}{Strongly connected} \\
    Плотность & \forall x,y \in M : (x \rel y) \implies \exists z \in M : (x \rel z) \land (z \rel y) & \href{https://en.wikipedia.org/wiki/Dense_relation}{Dense} \\
    \bottomrule
\end{tabular}
...
\begin{tabular}{l @{\hspace{1em}} L}
    ...
    \midrule
    Injective (left-unique) & \forall x,z \in X, y \in Y : (x \rel y) \land (z \rel y) \implies (x = z) \\
    Functional (right-unique) & \forall x \in X, y,z \in Y : (x \rel y) \land (x \rel z) \implies (y = z) \\
    One-to-One & \text{Injective and Functional} \\
    One-to-Many & \text{Injective and not Functional} \\
    Many-to-One & \text{Not Injective and Functional} \\
    Many-to-Many & \text{Not Injective and not Functional} \\
    Serial (left-total) & \forall x \in X : \exists y \in Y : (x \rel y) \\
    Surjective (right-total) & \forall y \in Y : \exists x \in X : (x \rel y) \\
    \bottomrule
\end{tabular}
...
\begin{terms}
    \item \textbf{Частичная функция} -- Functional бинарное отношение. Обозначается $f : X \pto Y$.
    \hfill\href{https://en.wikipedia.org/wiki/Partial_function}{Partial function}

    \item \textbf{Функция} -- Functional и Serial бинарное отношение. Обозначается $f : X \to Y$.
...
\begin{terms}
    \item \textbf{Отношение эквивалентности} -- рефлексивное, симметричное и транзитивное бинарное отношение.
    ...
    \item \textbf{Разбиение} (\textbf{Quotient set}) множества $A$ на классы эквивалентности: $\quotient[R]{A} = \equivclass{A} = \Set{\equivclass{x} \given x \in A}$.
...
\begin{terms}
    \item \textbf{Preorder} (\textbf{quasi-order}) -- \emph{reflexive} and \emph{transitive} relation.

    \item \textbf{Partial order} -- \emph{reflexive}, \emph{antisymmetric} and \emph{transitive} relation.

    \item \textbf{Total (linear) order} -- \emph{connex} partial order.

    \item \textbf{Strict partial order} -- \emph{irreflexive}, \emph{antisymmetric} and \emph{transitive} relation.

    \item \textbf{Strict total order} -- \emph{connex} strict partial order.
\end{terms}
...
\begin{terms}
    ...
    % R^{+}
    \item $R \union \bigunionclap{n \in N, n > 0} R^n$ -- \textbf{транзитивное замыкание} отношения $R \subseteq A^2$, где $R^1 = R$, $R^{k+1} = R^k \circ R$.
