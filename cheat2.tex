...
\begin{terms}
    ...
    \item $R^{-} = R \setminus \Set{\Pair{a,c} \given \exists b : (a \rel b) \land (b \rel c)}$ \--- \textbf{транзитивное сокращение} отношения $R \subseteq A^2$.
...
\begin{terms}
    ...
    \item Элемент упорядоченного множества $\Pair{M, R}$ называется \textbf{максимальным}, если он не меньше других элементов, то есть не существует элемента больше.
    Дуально, элемент называется \textbf{минимальным}, если он не больше других, то есть нет элемента меньше.
    \hfill\href{https://en.wikipedia.org/wiki/Maximal_and_minimal_elements}{Maximal and minimal elements}
    \begin{terms}
        \item $a \in M \text{ is \textbf{maximal}}
        \iff \forall b \neq a : \neg (a \rel b)
        ~\equiv~ \nexists b \neq a : (a \rel b)$
        \item $a \in M \text{ is \textbf{minimal}\,}
        \iff \forall b \neq a : \neg (b \rel a)
        ~\equiv~ \nexists b \neq a : (b \rel a)$
    \end{terms}

    \item Элемент упорядоченного множества $\Pair{M, R}$ называется \textbf{наибольшим}, если он больше всех элементов.
    Дуально, элемент называется \textbf{наименьшим}, если он меньше всех элементов.
    \hfill\href{https://en.wikipedia.org/wiki/Greatest_element_and_least_element}{Greatest and least elements}
    \begin{terms}
        \item $a \in M \text{ is \textbf{maximum} (\textbf{greatest})}
        \iff \forall b : (b \rel a)$
        \item $a \in M \text{ is \textbf{minimum} (\textbf{least})}
...
\begin{tabular}{@{} c @{}}
    ...
    \begin{tikzpicture}[
        ...
        \draw[arrow] (c) -- (ac);
        \draw[arrow] (c) -- (bc);
        \draw[arrow,frontline] (b) -- (ab);
        \draw[arrow,frontline] (b) -- (bc);
        \draw[arrow] (ab) -- (abc);
        \draw[arrow] (ac) -- (abc);
        \draw[arrow] (bc) -- (abc);
