...
#let dl-negative-cycle = canvas({
  ...
  let vpoint(pos, label, open: false) = {
    ...
    draw.content(
      (pos.at(0) - if open { 0.3 } else { 0 }, pos.at(1) + 0.42),
      anchor: "south",
      text(
        size: s-node,
        fill: c-ink,
  ...
  let guide(px, py, stroke: guide-stroke) = draw.line(
    (px, ax-y),
    (px, py),
    stroke: stroke,
  )

  draw.line((-2.0, ax-y), (5.9, ax-y), stroke: e-stroke, mark: (
    end: "stealth",
    fill: c-edge,
  ))
  ...
  draw.line((3, 1.0), (5.6, 1.0), stroke: hi-stroke, mark: (
    end: "stealth",
    fill: c-edge,
  ))
  draw.content((1.5, 1.42), text(size: s-cap, fill: c-ink)[$z >= x + 3$])
  draw.content((2.95, 1.42), anchor: "west", text(
    size: s-tiny,
    fill: c-muted,
  )[разрешено $z >= 3$])
  guide(3.5, 1.0)
  ...
  draw.line((3.5, 2.0), (4.5, 2.0), stroke: hi-stroke, mark: (
    end: "stealth",
    fill: c-edge,
  ))
  draw.content((3.3, 2.35), anchor: "east", text(
    size: s-cap,
    fill: c-ink,
  )[$w >= z + 1$])
  ...
  draw.line((-2.0, 3.0), (-2.9, 3.0), stroke: hot-stroke, mark: (
    end: "stealth",
    fill: c-hot,
  ))
  draw.content((0.1, 3.42), text(
    size: s-cap,
    fill: c-hot,
    weight: "bold",
  )[$w <= x + 2$])
  draw.content((0.1, 2.62), anchor: "north", text(
    size: s-tiny,
    fill: c-hot,
  )[разрешено $w <= 2$])
  draw.content((3.3, 3.0), text(size: 1.0em, fill: c-hot, weight: "bold")[✗])
  draw.content((3.3, 3.45), anchor: "south", text(
    size: s-cap,
    fill: c-hot,
    weight: "bold",
  )[противоречие])
