...
#proof[
  ...
  Каждому типу $sigma$ сопоставляется множество приводимых термов: для базового типа это (после подстановки) термы, редуцирующиеся к значению, а терм в $sigma -> tau$ приводим, если его результат при любом приводимом аргументе приводим в $tau$; нейтральные термы приводятся, как только приводятся их свободные переменные.
]

== Свойства: индукция по выводу

#proof[
  Индукцией по типам доказываются два свойства: каждый терм типа $sigma$ редуцируется к нормальной форме, и каждый терм типа $sigma$ с нормализующимися свободными переменными приводим в $sigma$.
