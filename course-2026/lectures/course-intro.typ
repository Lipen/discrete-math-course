...
#note[
  ...
  - Исходники книги, конспекты и слайды --- #link("https://github.com/Lipen/discrete-math-course")[в репозитории курса]
]
...
Четыре работы за семестр, до десяти баллов каждая.\
В каждой --- около десяти задач: базовые, челленджи и бонусные. Первая --- от руки.
...
#important[
  Домашняя работа приносит баллы: до 10 за модуль, 40 за семестр.\
  Пропущенные базовые задания или досдача после дедлайна --- не выше пяти за работу.
]
...
#important[
  Контрольная баллов не приносит --- она подтверждает домашнюю работу.\
  В зачёт модуля идёт меньшее из двух: домашки и контрольной.
]
...
#important[
  Экзамен не обязателен --- без него итог ограничен *80* баллами.\
  Провал --- ноль за экзамен.
]
...
#grid(
  ...
  tcard(c-red, [Контроли и экзамен])[
    Контрольные, теормины и экзамен --- запрещён.
  ],
)
...
#grid(
  ...
  column-gutter: 5pt,
  pseg(colors.accent, [40], [домашка и контрольные]),
...
#Block(color: colors.green)[
  ...
  Он по сегодняшней обзорной лекции, а не по домашнему чтению: чтение зададим после пары.
]
#v(1fr)
