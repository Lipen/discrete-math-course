...
#definition[Разбор][
  Дана строка $w$ и грамматика $G$; найти дерево разбора слова $w$ в $G$.
...
#important[
  Для LR-грамматик существуют генераторы парсеров: yacc, bison, ANTLR.
...
#definition[Рекурсивный спуск][
  Простейший нисходящий парсер: по одной функции на нетерминал, пишется вручную.

  Функция для $A$ пробует правые части правил $A -> alpha$ по очереди.
]

#note[
  Так устроены hand-written парсеры компиляторов.

  LL- и LR-языки --- собственные подклассы КС-языков: недетерминизм --- цена выразительности.
]
...
#important[
  Подъём по иерархии платит алгоритмическими гарантиями.
...
Стек --- память только сверху; КЗ-языкам нужна память с произвольным доступом.

#definition[Линейно-ограниченный автомат][
  Машина Тьюринга, длина ленты которой ограничена линейной функцией от длины входа.
...
#important[
  ...
  - Стек --- память о глубине вложенности; КС-языки --- ровно те, что распознаются МП-автоматом.
