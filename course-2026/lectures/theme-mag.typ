...
// Модульный акцент: --input mod=<поток>; без input --- базовый сине-индиго.
#let _mods = (
...
#let _mod-key = if "mod" in sys.inputs { sys.inputs.mod } else { none }
#let _acc = if _mod-key != none { _mods.at(_mod-key, default: none) } else { none }
#let colors = if _acc != none {
  (.._base, accent: _acc, accent-strong: _acc.darken(8%))
} else {
  _base
}

// Имена модулей для фолио; индекс в семестре (0..3) для точек на титуле.
#let _names = (
  sets: "Множества",
  relations: "Отношения",
    ...
    #v(1.4em, weak: true)
      ...
      #block(
        ...
        #set text(1.4em, style: "italic", fill: colors.ink)
          ...
          #v(0.8em, weak: true)
          #align(right)[
            #text(0.8em, weight: "bold", fill: colors.accent-strong)[--- #epigraph-author]
          ]
        ]
      ]
    ]
...
#let lecture-slide(num, title, week: none, recap: none) = {
  ...
  place(right + top, dx: -1.1cm, dy: 0.8cm)[
    #text(8em, weight: "bold", fill: white.transparentize(60%), font: "Libertinus Sans")[#num]
  ]

  // Единый вертикальный поток: заголовок и рекап не могут столкнуться
  place(left + horizon, block(width: 100%, inset: (x: 2cm, y: 0.5cm))[
    #stack(
      dir: ttb,
      spacing: 1em,
      text(0.8em, weight: "bold", tracking: 0.25em, fill: white.transparentize(15%))[ЛЕКЦИЯ],
      block(width: 92%)[
        #set par(leading: 0.5em)
        #text(3em, weight: "bold", font: "Libertinus Sans", fill: white)[#title]
      ],
      if week != none [
        #box(
      ...
      if recap != none [
        #block(
          ...
          #text(fill: colors.accent-strong, weight: "bold")[На прошлой лекции:]
          #v(0.8em, weak: true)
  ...
  if title != none {
    ...
    title-slide({
        ...
        place(center + horizon, block(width: 88%, inset: (x: 1cm))[
          #align(center)[
            #text(0.8em, weight: "bold", tracking: 0.25em, fill: colors.muted)[ДИСКРЕТНАЯ МАТЕМАТИКА]
            #v(0.8em, weak: true)
            #text(3em, weight: "bold", font: title-font, fill: title-color)[#title]
            #v(1em, weak: true)
            ...
            #if _mod-key != none and _mod-key in _mod-idx [
              #v(1em, weak: true)
              #box(inset: (y: 0.4em))[
                #let active = _mod-idx.at(_mod-key)
                #for i in range(4) {
                  if i > 0 { h(0.8em) }
                  box(circle(
                    radius: 4pt,
                    fill: if i == active { colors.accent },
