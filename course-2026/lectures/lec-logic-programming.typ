...
#important[
  Обычная программа берёт данные, выполняет операции и выдаёт результат. Логическая программа описывает, что истинно, а машина по этому описанию ищет доказательство.
]
...
#note[
  ...
  Рекурсия идёт по структуре списка: обрабатываем голову, а хвост отдаём рекурсивному вызову. Корректность доказывается индукцией по длине списка.
]
