...
#example[
  $p or not p$ is a tautology (the law of excluded middle).
  $p and not p$ is a contradiction.
  $p and q$ is satisfiable but not a tautology.
]
...
#definition[Disjunctive Normal Form (DNF)][
  A formula is in DNF if it is a disjunction of conjunctions of literals (atoms or their negations):
...
#example[
  With domain $NN$:
  $forall x space x >= 0$ is true.
  $exists x space x < 0$ is false.
  $forall x exists y space y > x$ is true (the natural numbers are unbounded).
]
...
$exists y forall x$ says "there is a single $y$ that works for all $x$."

#remark[
  Quantifier order is the logical analogue of loop nesting.
  $forall x exists y$ is like a nested loop where the inner computation depends on the outer index.
  $exists y forall x$ is like precomputing a value that works for the whole iteration.
]
