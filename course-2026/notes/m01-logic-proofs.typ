...
#example[Truth table for $p imply q$][
  #table(
columns: 3, align: center, stroke: (x, y) => if y == 0 { (bottom: 0.4pt) }, table.header([$p$], [$q$], [$p imply q$]), [#Green([T])], [#Green([T])], [#Green([T])], [#Green([T])], [#Red([F])], [#Red([F])], [#Red([F])], [#Green([T])], [#Green([T])], [#Red([F])], [#Red([F])], [#Green([T])], ) ]
