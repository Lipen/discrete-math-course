...
IV век до н.э.
Аристотель в «Органоне» формулирует первую формальную теорию рассуждений --- силлогистику.
Он выделяет схемы правильных умозаключений и классифицирует их по фигурам и модусам.
Это первая попытка отделить структурно корректное рассуждение от содержательно убедительного --- и она остаётся стандартом логического анализа на два тысячелетия.

1666 год.
Готфрид Вильгельм Лейбниц, девятнадцати лет от роду, публикует «De Arte Combinatoria» --- трактат, в котором впервые высказана идея механического рассуждения.
Он предлагает calculus ratiocinator --- исчисление, которое позволило бы разрешать любые споры вычислением: «Давайте посчитаем!»
Идея опередила технические возможности эпохи на два с половиной столетия.

1847 и 1854 годы.
Джордж Буль публикует «The Mathematical Analysis of Logic» и «An Investigation of the Laws of Thought», в которых алгебраизирует логику: логические операции становятся алгебраическими --- конъюнкция умножением, дизъюнкция сложением.
Булева алгебра превращает рассуждение в вычисление --- ровно то, о чём мечтал Лейбниц, но на прочном математическом фундаменте.

1879 год.
Готлоб Фреге в «Begriffsschrift» (понятийном письме) вводит логику предикатов с кванторами --- первую формальную систему, достаточно выразительную для записи произвольных математических утверждений.
Его нотация --- двумерные диаграммы --- не прижилась, но логическая конструкция стала стандартом.

1910--1913 годы.
Бертран Рассел и Альфред Уайтхед в трёхтомных «Principia Mathematica» предпринимают грандиозную попытку свести всю математику к логике.
Работа колоссальна: доказательство того, что $1 + 1 = 2$, занимает несколько сотен страниц.
Проект показал принципиальную возможность формализации --- и её практическую неподъёмность без механизации.

1931 год.
Курт Гёдель доказывает теоремы о неполноте: любая достаточно выразительная формальная система либо противоречива, либо неполна --- в ней существуют истинные, но недоказуемые утверждения.
Формализация не всесильна.
У логики есть пределы, и они не временные, а принципиальные.

1936 год.
Алан Тьюринг формализует понятие алгоритма через машину Тьюринга и связывает логику с вычислениями: проблема разрешения истинности логических формул оказывается эквивалентной проблеме остановки машины.
Логика и вычисление --- две стороны одного явления.
Именно на этом стыке возникнут и теория алгоритмов, и современная информатика, и формальная верификация программ.

Путь от аристотелевых силлогизмов до формальной верификации программ занял два тысячелетия.
Мы пройдём его за одну главу --- сосредоточившись на инструментах, которые работают, и на том, как их применять.
...
#remark[
  ...
  Именно на ней держится доказательство корректности компиляторов (CompCert --- верифицированный компилятор C, сертифицированный с помощью Coq) и свойств систем типов (progress и preservation в доказательстве type safety языка с формальной семантикой).

  #footnote[CompCert: X. Leroy et al., "CompCert --- A Formally Verified Optimizing Compiler", INRIA, 2005--2025. Доказательство type safety: B. Pierce, "Types and Programming Languages", MIT Press, 2002, глава 8.]
]
...
#remark[
  ...
  #footnote[Dafny: K. R. M. Leino, "Dafny: An Automatic Program Verifier for Functional Correctness", Microsoft Research, 2010. Why3: J.-C. Filliâtre, A. Paskevich, "Why3 --- Where Programs Meet Provers", ESOP 2013. Frama-C: CEA LIST, "Frama-C User Manual", 2008--2025.]
]
