...
#figure(
  parse-tree-imply,
  caption: [Дерево разбора формулы $not p and q imply r$. Приоритет связок определяет структуру: $not$ связывает $p$ (сильнее всего), $and$ соединяет $not p$ и $q$, а $imply$ соединяет результат с $r$. Без соглашений о приоритетах пришлось бы писать $(((not p) and q) imply r)$.],
