// M12 (logic programming) diagrams: SLD search tree.
...
// SLD-дерево для запроса `?- ancestor(alice, Y)`.
// Программа: факты parent(alice, bob), parent(bob, carol);
// правило 1 (база): ancestor(X, Y) :- parent(X, Y);
// правило 2 (рекурсия): ancestor(X, Y) :- parent(X, Z), ancestor(Z, Y).
// Поиск в глубину спускается по левой ветви до ответа, затем по правой.
#let sld-tree = canvas({
