...
#figure(
  ...
  caption: [
    ...
    На каждой диаграмме: левый овал --- область определения (домен), правый овал --- область значений (кодомен).
    Стрелки показывают, какой выход $f(x)$ соответствует каждому входу $x$.
  ],
) <fig:mapping-types>
