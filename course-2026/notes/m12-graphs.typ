...
#proposition[Minimum spanning tree][
  ...
  - *Kruskal*: sort edges by weight, add lightest that does not create a cycle (union-find).\
    $O(E log E)$.
  - *Prim*: grow tree from arbitrary start, repeatedly add lightest edge to outside vertex (priority queue).\
    $O((V+E) log V)$.
]
