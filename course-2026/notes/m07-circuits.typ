...
#definition[Circuit family][
  A sequence $C_1, C_2, ...$ where $C_n$ has $n$ inputs.
  *Uniform* if an algorithm constructs $C_n$ from $n$.
...
#definition[Ripple-carry adder][
  ...
  Carry propagates through all stages: size $O(n)$, depth $O(n)$.
  *Carry look-ahead* reduces depth to $O(log n)$ by computing carries in parallel.
]
