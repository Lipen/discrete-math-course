...
#figure(
hasse-divisors-12, caption: [Hasse diagram of $(D_12, |)$ --- the divisor lattice of 12.
Edges show cover relations; transitivity and reflexivity are implicit.], ) <fig:hasse-divisors-12>

#figure(
hasse-powerset-2, caption: [Hasse diagram of $(cal(P)({1, 2}), subset.eq)$ --- the Boolean lattice $B_2$.
Join = $union$, meet = $inter$.], ) <fig:hasse-powerset-2>
...
#proposition[Extremal elements --- comparison][
  #table(
columns: 4, align: (left, center, center, left), stroke: (x, y) => if y == 0 { (bottom: 0.4pt) }, table.header( [*Concept*], [*Definition*], [*Unique?*], [*Exists in finite poset?*] ), [Minimal], [$not(exists a: a < m)$], [No --- can be many], [Always], [Maximal], [$not(exists a: m < a)$], [No --- can be many], [Always], [Least], [$m prec.eq a$ for all $a$], [Yes], [Not guaranteed], [Greatest], [$a prec.eq m$ for all $a$], [Yes], [Not guaranteed], ) ]
