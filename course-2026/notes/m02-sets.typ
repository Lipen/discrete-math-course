...
#proposition[Set--logic correspondence][
  #table(
columns: 2, align: (left, left), stroke: (x, y) => if y == 0 { (bottom: 0.4pt) }, table.header([*Set operation*], [*Logical counterpart*]), [$A union B$], [$x in A or x in B$], [$A inter B$], [$x in A and x in B$], [$overline(A)$], [$not(x in A)$], [$A setminus B$], [$x in A and not(x in B)$], [$A symdiff B$], [$x in A xor x in B$], [$A subset.eq B$], [$x in A imply x in B$], [$A = B$], [$x in A iff x in B$], ) ]
