# crypto

Теория чисел и криптография.

Небольшой набор инструментов модулярной арифметики на `u64` без зависимостей, учебный RSA с подписью и набор атак, которые действительно его взламывают. Смысл крейта: чтобы доверять криптосистеме, сначала нужно её атаковать.

## Быстрый старт

```bash
cargo run -p crypto --example rsa_demo
cargo run -p crypto --example pohlig_hellman
cargo test -p crypto
```

## Модули

| Модуль | Назначение |
| --- | --- |
| `modular` | `gcd`, `egcd`, `mod_inverse`, `mod_pow` |
| `rsa` | Структура `Rsa`: генерация ключей, шифрование/дешифрование, подпись/проверка |
| `attacks` | Общий модуль, податливость, факторизация, китайская теорема об остатках, Pohlig--Hellman |

## RSA на одной картинке

![Схема RSA](assets/rsa-flow.svg)

Генерация ключей выбирает простые числа $p$, $q$, вычисляет $n = pq$ и $\phi(n) = (p-1)(q-1)$, выбирает открытую экспоненту $e$, взаимно простую с $\phi(n)$, и выводит закрытый ключ $d = e^{-1} \bmod \phi(n)$. Шифрование и дешифрование --- это одна и та же операция с разными экспонентами:

$$ c = m^e \bmod n \qquad m = c^d \bmod n $$

## API

| Элемент | Назначение |
| --- | --- |
| `gcd`, `egcd`, `mod_inverse`, `mod_pow` | Набор инструментов модулярной арифметики |
| `Rsa::new`, `encrypt`, `decrypt` | Учебный RSA на малых числах |
| `Rsa::sign`, `verify` | Цифровая подпись с RSA |
| `attacks::common_modulus_attack` | Восстановить $m$ из двух шифротекстов, имеющих общий $n$ |
| `attacks::malleable_product` | $c_1 c_2 = (m_1 m_2)^e \bmod n$ |
| `attacks::factorize` | Пробное деление; показывает, почему малые простые числа не подходят |
| `attacks::crt` | Китайская теорема об остатках |
| `attacks::pohlig_hellman` | Дискретный логарифм, когда $p-1$ гладкое |

## Демо

| Демо | Идея | Что печатает |
| --- | --- | --- |
| `brute_force_caesar` | Шифр Цезаря | Все 32 сдвига; открытый текст появляется при сдвиге 3 |
| `frequency_analysis` | Простая замена | Частоты букв; самая частая буква шифротекста соответствует "о" |
| `common_modulus` | Атака с общим модулем на RSA | $m$ восстановлено без какого-либо закрытого ключа |
| `malleability` | Податливость RSA | $c_1 c_2$ расшифровывается в $m_1 m_2$ |
| `pohlig_hellman` | Дискретный логарифм, гладкий порядок | $x$ такое, что $g^x = h \pmod p$ |
| `rsa_demo` | RSA | Полный цикл шифрования/дешифрования и подпись/проверка |

## Тесты

```bash
cargo test -p crypto
```
