...
Они проверяют форму НФХ (каждая продукция — это $A \to B C$, $A \to a$ или восстановленная $S \to \varepsilon$), сохранение языка при преобразовании, ячейки таблицы CYK для `aabb`, число деревьев неоднозначной грамматики ($S \to S S \mid a$ даёт два дерева для `aaa`) и предел, превращающий ε-цикл в `GrammarError::TooManyParseTrees`.
