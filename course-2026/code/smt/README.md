...
A small DPLL(T) stack for quantifier-free SMT problems, built in layers from a Boolean core to a mixed-signature driver.
...
The driver is deliberately simple: one theory per atom, no Nelson–Oppen combination of variables across theories.
Linear equality atoms are rejected because their negation is a disjunction: write `sum == b` as the two atoms `sum <= b` and `-sum <= -b`.
Each theory documents its own precision limits.
`linear` and `integers` keep all arithmetic exact but expect small coefficients, since elimination multiplies them.
`bitvec` supports widths from 1 to 64 bits with the operations `Not`, `And`, `Or`, `Add`, `Sub`, `Extract`, `Eq`, `Neq`.
...
A mixed formula is a `Formula` over `Atom`s of the four theories:
