...
\begin{terms}
    ...
    \item Two graphs $G_1 = (V_1, E_1)$ and $G_2 = (V_2, E_2)$ are called \textbf{isomorphic}\Href{https://en.wikipedia.org/wiki/Graph_isomorphism}, denoted $G_1 \isomorphic G_2$, if there exists an \emph{edge-preserving} bijection $f \colon V_1 \to V_2$, \ie any two vertices $u,v \in V_1$ are adjacent in~$G_1$ if and only if $f(u)$ and~$f(v)$ are adjacent in~$G_2$.
