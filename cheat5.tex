...
\MakeSingleHeader% {<l>}{<r>}
    ...
    {\TextDiscreteMathEng, \IconSpring~Spring 2023}
\fancyfoot{}
...
\begin{terms}
    ...
    \begin{tikzpicture}[
        baseline,
        on grid,
        matching/.style={mygreen,line width=3pt},
        notmatching/.style={myred,line width=2.5pt},
    ]
        \def\Side{0.6}
        \def\HGap{2.5cm}
        \def\LabelX{1.5*\Side}
    ...
    \begin{tikzpicture}[
        baseline,
        on grid,
        % cover/.style={dot,minimum size=6pt,fill=mygreen},
        cover/.style={
            circ,
            minimum size=11pt,
            fill=mygreen,
        ...
        % covered/.style={mygreen,line width=2pt},
        notcovered/.style={myred,line width=2pt},
    ]
        \def\Side{0.8}
        \def\HGap{2.5cm}
        \def\LabelX{\Side}
    ...
    \begin{terms}
        \item Two edges are called \emph{biconnected} if there exist two \emph{edge-disjoint} paths between the ends of these edges.
        \item Trivially, this relation is an equivalence relation.
        \item Equivalence classes of this relation are called \textbf{biconnectivity components}\Href{https://en.wikipedia.org/wiki/Biconnected_component}, also known as \textbf{blocks}.
    \end{terms}

    \item \textbf{Edge biconnectivity} can be defined as a relation on vertices $R \subseteq V^2$:
    \begin{terms}
        \item Two vertices are called \emph{edge-biconnected} if there exist two \emph{internally vertex-disjoint} paths between them.
        \item Trivially, this relation is an equivalence relation.
        \item Equivalence classes of this relation are called \textbf{edge biconnectivity components} (or \emph{2-edge-connected components}).
    \end{terms}
