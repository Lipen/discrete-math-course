...
\begin{tasks}
    ...
    For each case, provide representative examples of (non-)complying numbers (\eg 12345 and 52814 are suitable for~\ref{item:digits-cannot-be-repeated}, but 44521 and 935 are~not) and derive a generic\footnote{Here, \enquote{generic formula} means \enquote{depending on the input data}. In this particular example, $n = 9$ and $k = 5$, but the sought formula must also be valid for all other (adequate) values of~$n$ and~$k$.} formula.
